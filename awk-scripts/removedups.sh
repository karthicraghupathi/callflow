...
done |awk '{print $1}' | sort -n  > $TMPDIR/pkgs

#echo Trick on callflow.short
sed 's/^ *//' $DESTDIR/callflow.short > $TMPDIR/callflow.short.dup

# Join the files
for i in $(cat $TMPDIR/pkgs); do
  grep "^$i " $TMPDIR/callflow.short.dup
done > $DESTDIR/callflow.short.new

#Remove temp files
rm -f $TMPDIR/md5sums
rm -f $TMPDIR/pkgs
rm -f $TMPDIR/callflow.short.dup
